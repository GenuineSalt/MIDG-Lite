...
void MaxwellsRun3d(Mesh *mesh, double FinalTime, double dt, double *Hx, double *Hy, double *Hz, double *Ex, double *Ey, double *Ez, float *vgeo, float *mapinfo, float *surfinfo){
    ...
    for(tstep=0;tstep<Nsteps;++tstep){
        ...
        for (INTRK=1; INTRK<=5; ++INTRK) {  
            ...
            surfaceKernel(mesh, Q, partQ, rhsQ, mapinfo, surfinfo);
...
void surfaceKernel(Mesh *mesh, float *Q, float *partQ, float *rhsQ, float *mapinfo, float *surfinfo) {
    ...
    for (int k = 0; k < mesh->K; ++k) {
        ...
        float Hx_flux[p_Nfaces*p_Nfp];
        float Hy_flux[p_Nfaces*p_Nfp];
        float Hz_flux[p_Nfaces*p_Nfp];
        float Ex_flux[p_Nfaces*p_Nfp];
        float Ey_flux[p_Nfaces*p_Nfp];
        float Ez_flux[p_Nfaces*p_Nfp];
        ...
        for (int m = 0; m < p_Nfaces*p_Nfp; ++m) {
            ...
            Hx_flux[m] = -ny*dEz + nz*dEy + dHx - ndotdH*nx;
            Hy_flux[m] = -nz*dEx + nx*dEz + dHy - ndotdH*ny;
            Hz_flux[m] = -nx*dEy + ny*dEx + dHz - ndotdH*nz;
            Ex_flux[m] = ny*dHz - nz*dHy + dEx - ndotdE*nx;
            Ey_flux[m] = nz*dHx - nx*dHz + dEy - ndotdE*ny;
            Ez_flux[m] = nx*dHy - ny*dHx + dEz - ndotdE*nz;
        }
        ...
        for (int n = 0; n < p_Np; ++n) {
            ...
            for (int m = 0; m < p_Nfp; ++m) {
                ...
                for (int f = 0; f < p_Nfaces; ++f) {
                    ...
                    rhsHx += mesh->LIFT[n][m*p_Nfaces + f]*Hx_flux[m*p_Nfaces + f];
                    rhsHy += mesh->LIFT[n][m*p_Nfaces + f]*Hy_flux[m*p_Nfaces + f];
                    rhsHz += mesh->LIFT[n][m*p_Nfaces + f]*Hz_flux[m*p_Nfaces + f];
                    rhsEx += mesh->LIFT[n][m*p_Nfaces + f]*Ex_flux[m*p_Nfaces + f];
                    rhsEy += mesh->LIFT[n][m*p_Nfaces + f]*Ey_flux[m*p_Nfaces + f];
                    rhsEz += mesh->LIFT[n][m*p_Nfaces + f]*Ez_flux[m*p_Nfaces + f];
                }
            }
